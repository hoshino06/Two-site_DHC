...
% InputPort(1): xi states
block.InputPort(1).Dimensions  = 9;
block.InputPort(1).DatatypeID  = 0;  % double
block.InputPort(1).Complexity  = 'Real';
block.InputPort(1).DirectFeedthrough = true;

% InputPort(2): eta states
block.InputPort(2).Dimensions  = 4;
block.InputPort(2).DatatypeID  = 0;  % double
block.InputPort(2).Complexity  = 'Real';
block.InputPort(2).DirectFeedthrough = true;

% InputPort(3): Desired output trajectory
block.InputPort(3).Dimensions  = (horizon+1);
...
% InputPort(5): Current state
block.InputPort(5).Dimensions  = 13;
...
x =  block.InputPort(5).Data; %disp(x')
...
% Linearization for the dynamics of z_eta
x_bar     = zeros(13,Tnum+1); % nominal trajectory
z_bar     = zeros(9, Tnum+1);  
z_eta_bar = zeros(4, Tnum+1);
F_z   = zeros(4,9,Tnum);      % linearized system
...
% cost fucntion
z_k = block.InputPort(1).Data; % current state z[k]
Qz = eye(9);
Rv = eye(2);
Pz = idare(F,G,Qz,Rv,[],[]);
Q = blkdiag( eye( 9*Tnum), Pz  ); 
R = eye( 2*Tnum );
H = 2 * ( FG'* Q * FG + R ); 
...
if x(1) == 0.23 % at initial step
   ...
   A = []; b = [];
    ...
    % constraints of input u
    A_u = zeros( 2* Tnum, 2*Tnum ); 
    b_u = zeros( 2* Tnum, 1); 
    for k = 1:Tnum
       % upper bound
       u_max = [1.0; 1.0]; 
       A_u(2*k-1:2*k, 2*k-1:2*k) = inv(decoupling_matrix(x_bar(:,k)));
       b_u(2*k-1:2*k, 1) = u_max + decoupling_matrix(x_bar(:,k)) \ decoupling_shift_term(x_bar(:,k)); 
    end
   
    % constrains of det(dPhidt)
    Iper = zeros(13*(Tnum+1));
    for k = 0:Tnum
        Iper( 1+13*k:13+13*k, 1+9*k:9+9*k ) = [eye(9); zeros(4,9)];
        Iper( 1+13*k:13+13*k, 9*(Tnum+1)+1+4*k: 9*(Tnum+1)+4+4*k ) = [zeros(9,4); eye(4)];
    end
    MatP = zeros(Tnum,13*(Tnum+1));    
    for k = 1:Tnum
        %deriv_det_dphidx(x_bar(:,k+1))' 
        %deriv_det_dphidx(x_bar(:,k+1))' / ve_dphidx(x_bar(:,k+1))
        MatP(k,1+13*k:13+13*k) = deriv_det_dphidx(x_bar(:,k+1))' / ve_dphidx(x_bar(:,k+1));
    end
    A_phi = MatP * Iper * [FG; FG_z*FG+FG_v];
    XI_z = Iper*( [ FF zeros( 9*(Tnum+1),4 ); FG_z*FF FF_eta]*[z_k; z_eta_k] - [zeros(9*(Tnum+1),1); ZZ_eta_bar]);
    XI_bar = zeros(13*(Tnum+1),1);
    for k = 0:Tnum
       XI_bar(1+13*k:13+13*k) = [z_bar(:,k+1); z_eta_bar(:,k+1)];  
    end
    Lp  = 0; % Lower bound of det(dPhidx)
    b_phi = Lp*ones(Tnum,1) - MatP*( XI_z - XI_bar );
