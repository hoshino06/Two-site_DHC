...
csvdata = pd.read_csv('simex2_constraint_outputs.csv')
...
csvdata = pd.read_csv('simex2_weight_Qe10_Qh100_outputs.csv')
...
csvdata = pd.read_csv('simex2_weight_Qe100_Qh10_outputs.csv')
...
# データのプロット
plt.plot(time_case1, h1_case1,    label='original setting', zorder=2, ls='-', lw=1.5, c='g')
plt.plot(time_case2, h1_case2, label='higher weight on y2', zorder=2, ls='-', lw=1.5, c='orangered')
plt.plot(time_case3, h1_case3, label='higher weight on y1', zorder=2, ls='-', lw=1.5, c='b')
plt.plot(time_case1, pref,  label='',  zorder=3, ls=':',  lw=2, c='r')
# 軸ラベルの設定
...
plt.legend(loc='lower right', bbox_to_anchor=(0.99,0.02), #凡例の位置
...
plt.plot(time_case2, h2_case2, label='h2', zorder=1, ls='-', lw=1.5, c='orangered')
...
plt.plot(time_case2, h3_case2, label='', zorder=3, ls='-', lw=1.5, c='orangered')
...
plt.savefig('simex2_weight_outputs.pdf', bbox_inches="tight")
