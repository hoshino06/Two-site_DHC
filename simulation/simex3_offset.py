...
csvdata = pd.read_csv('simex3_offset00.csv')
...
csvdata = pd.read_csv('simex3_offset04.csv')
time_case2 = csvdata['time']
h1_case2 = Pbase*csvdata['power']
h2_case2 = Qbase*csvdata['heatflow']
h3_case2 = pbase*csvdata['pressure']/1000 + 800

csvdata = pd.read_csv('simex3_offset08.csv')
...
# データのプロット
plt.plot(time_case1, h1_case1,    label='$Y_2^\mathrm{offset}=0.0$', zorder=2, ls='-', lw=1.5, c='b')
plt.plot(time_case2, h1_case2, label='$Y_2^\mathrm{offset}=0.4\,\mathrm{MJ/s}$', zorder=2, ls='-', lw=1.5, c='r')
plt.plot(time_case3, h1_case3, label='$Y_2^\mathrm{offset}=0.8\,\mathrm{MJ/s}$', zorder=2, ls='-', lw=1.5, c='g')
...
# データのプロット 
plt.plot(time_case1, h2_case1, label='h2', zorder=1, ls='-', lw=1.5, c='b')
plt.plot(time_case2, h2_case2, label='h2', zorder=1, ls='-', lw=1.5, c='r')
plt.plot(time_case3, h2_case3, label='h2', zorder=1, ls='-', lw=1.5, c='g')
...
# データのプロット 
plt.plot(time_case1, h3_case1, label='', zorder=3, ls='-', lw=1.5, c='b')
plt.plot(time_case2, h3_case2, label='', zorder=3, ls='-', lw=1.5, c='r')
...
plt.savefig('simex3_offset.pdf', bbox_inches="tight")
