...
% MPCの評価関数の重み
Qz = 'blkdiag( 500, eye(4), 100, eye(3) )'; % パラメータは文字列で指定
Rv = '0.01*eye(2)';
...
%% シミュレーションの実行
%%%%%%%%%%%%
Qz = 'blkdiag( 100, eye(4), 10, 100*eye(3) )'; % パラメータは文字列で指定
set_param([mdl,'/mpc'], 'Qz', Qz)
...
writetable(T, 'simex2_weight_Qe100_Qh10_outputs.csv')
T = array2table( ...
...
writetable(T, 'simex2_weight_Qe100_Qh10_inputs_etc.csv')


%%%%%%%%%%%%%%%%%%%%
Qz = 'blkdiag( 10, eye(4), 100, eye(3) )'; % パラメータは文字列で指定
set_param([mdl,'/mpc'], 'Qz', Qz)
...
T = join(T, array2table(res.heatflow, 'VariableNames',{'time','heatflow'}) );
T = join(T, array2table(res.pressure, 'VariableNames',{'time','pressure'}) );
writetable(T, 'simex2_weight_Qe10_Qh100_outputs.csv')
T = array2table( ...
    [res.inputs.time, res.inputs.signals.values], 'VariableNames',{'time','in1','in2'} );
T = join(T, array2table( ...
    [res.cal_time.time res.cal_time.signals.values], ...
    'VariableNames',{'time','cal_time'}) );
writetable(T, 'simex2_weight_Qe10_Qh100_inputs_etc.csv')
